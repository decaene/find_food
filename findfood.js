...
router.post("/get_facilidades",function(req,res){
    var collection    =  datb.collection('Facilidades');
    collection.aggregate([
    ]).toArray(function(err, result){  
        if(err){
            var res_err      = {};
...
router.post("/autenticacion",function(req,res){
    ...
    collection.aggregate([
		...
		{ $lookup: { from: "Ubicacion", localField: "_id", foreignField: "usuario_id", as: "ubicaciones" } },
		{ $unwind: { path: "$restaurantes", preserveNullAndEmptyArrays: true } },
		{ $lookup: { from: "Tipo_Restaurante", localField: "restaurantes.tipo_restaurante_id", foreignField: "_id", as: "restaurantes.tipo_restaurante" } },
		{
		  $group: {
			"_id" : "$_id",
			"nombre" : { $first: "$nombre" },
                    ...
                    if(err){
						...
						if(foto.includes("data")){
							...
							collection.update(
								...
								{ $set: { 'foto' : 'http://165.227.30.166:3025/usuario/'+result_usuario_res.insertedIds[0]+'_foto.png' } }, 
								function(err, result2){  
									if(err){
										var res_err      = {};
										res_err.status   = "error"; 
				...
				function(err, result2){  
					if(err){
						...
						if(foto.includes("data")){
							...
							collection.update(
								{ '_id' : usuario_id }, 
								{ $set: { 'foto' : 'http://165.227.30.166:3025/usuario/'+req.body.data._id+'_foto.png' } }, 
								function(err, result2){  
									if(err){
										var res_err      = {};
										res_err.status   = "error";
    ...
    collection.update(
		...
		function(err, result){  
			if(err){
				...
				var res_err      = {};
				res_err.status   = "success";
				res_err.foto   	 = 'http://165.227.30.166:3025/usuario/'+req.body.data._id+'_foto.png'
...
router.post("/actualizar_empresa",function(req,res){
		...
		collection.update(
				...
				if(foto.includes("data")){
					...
					collection.update(
						{ '_id' : empresa_id }, 
						{ $set: { 'foto' : 'http://165.227.30.166:3025/usuario/'+req.body.empresa._id+'_foto.png' } }, 
						function(err, result2){  
							if(err){
								var res_err      = {};
								res_err.status   = "error";
...
router.post("/actualizar_restaurante_direccion",function(req,res){
		var collection		=  datb.collection('Restaurante');
		var restaurante_id	=  ObjectId(req.body.restaurante._id);
		collection.update(
					{ '_id' : restaurante_id }, 
					{ $set: { 	
						"latitude"  : req.body.restaurante.latitude,
						"longitude" : req.body.restaurante.longitude,
						"direccion" : req.body.restaurante.direccion,
						"location"  : { "type" : "Point" , "coordinates" : [ req.body.restaurante.latitude, req.body.restaurante.longitude ] },
						"loc"  : [ req.body.restaurante.latitude, req.body.restaurante.longitude ] 
					} }, 
					function(err, result2){  
						if(err){
							var res_err      = {};
				...
				var result_return      = {};
				result_return.status   = "success";
				result_return.message  = "Dirección actualizada :)";
				res.send(result_return);
			}
		});
});
    ...
    collection.insert(req.body.empresa, function(err, result) {
        if(err){
            ...
            collection.update(
                ...
                { $set: { 'foto' : 'http://165.227.30.166:3025/empresa/'+result.insertedIds[0]+'_foto.png' } }, 
                function(err, result2){  
                    if(err){
                        var res_err      = {};
                        res_err.status   = "error"; 
